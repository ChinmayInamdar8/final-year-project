...
export function ExamBoard({no_of_questions, exam_id}:{no_of_questions:number, exam_id:string}) {
  ...
  useEffect(()=>{
    setLoading(true);
    axios.post('/api/exam/assessment/get-question', {
    ...
    <div className="w-full h-[92vh] flex flex-col overflow-hidden">
      ...
      <div className=" flex justify-between mt-4 items-center">
        <div className="bg-slate-600 rounded px-8 ml-3 text-white">
          ...
          <p className="text-center">1:30:00</p>
        </div>
        <div className="bg-sky-600 py-1 rounded px-8 ml-3 text-white">
          Exam ID : 123456
        </div>
        <div className="mr-4">
          <button className="px-8 rounded py-1 text-xl text-white bg-red-600"
          onClick={async()=>{
            const res = await axios.post("/api/exam/assessment/check-result", {
            ...
            if(res.data.message==="done"){
                router.push(`/route/give-assessment/result?m=${res.data.data.marks}`)
            }
          }}
          >
            Submit
      ...
      <div className="w-full h-4/5 grid grid-cols-10 mt-3">
        <div className="col-span-1 h-full bg-slate-200 ml-1 mr-2 rounded shadow-lg overflow-y-auto overflow-x-hidden border-2 border-sky-600 flex flex-col items-center">
          {[...Array(no_of_questions)].map((_, key)=>(
            <QuestionNumber key={key} ques_no={key+1} setSelected_question_no={setSelected_question_no} selected_question_no={selected_question_no}></QuestionNumber>
          ))}
        </div>
        <div className="col-span-9 h-full bg-slate-200 ml-2 mr-3 rounded shadow-xl overflow-y-auto overflow-x-hidden border-l-8 border-green-600 flex flex-col items-center">
          ...
          <ActualQuestion exam_id={exam_id} question={question} actualOptions={options} handleOptionSelect={handleOptionSelect} selected={selected_options[selected_question_no-1] || ""}></ActualQuestion>
        </div>
      </div>
    </div>
  );
...
const QuestionNumber = ({ques_no, setSelected_question_no, selected_question_no}:{ques_no:number, setSelected_question_no:Dispatch<SetStateAction<number>>, selected_question_no:number}) => {
  return (
    <button className={`py-1 px-2 border border-sky-600 rounded  my-2 ${
      selected_question_no==ques_no ? "bg-red-600 text-white" : "bg-orange-200"
    }`} 
...
const ActualQuestion = ({ exam_id, 
                          question, 
                          actualOptions, 
                          handleOptionSelect,
                          selected}: TypeActualQuestion) => {
  ...
  return (
    <div className="w-full flex flex-col mt-7">
        ...
        {options.map((opt) => (
          <label
            ...
            className={`my-3 flex justify-start px-4 items-center w-full border-2 ${
              selected === opt.id ? "border-blue-800 bg-blue-100" : "border-sky-600"
      ...
      <div className="w-full flex justify-between mb-5">
        <button className="mx-6 py-1 px-8 rounded text-white bg-green-600">Previous</button>
        <button className="mx-6 py-1 px-8 rounded text-white bg-violet-600">Next</button>
      </div>
    </div>
  );
};
